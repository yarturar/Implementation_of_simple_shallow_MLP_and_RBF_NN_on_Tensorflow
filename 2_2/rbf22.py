...
def rbf22(data, n_hidden, rho, sigma): 
    ...
    random.seed(1820436)
    centers = np.random.choice(X_train[1], (n_input,n_hidden))
    ...
    c_2=tf.reduce_sum(tf.square(centers), 0)
    x2upd=tf.reshape(x_2, [-1, 1])
    c2upd=tf.reshape(c_2, [1, -1])
    
    r=x2upd-2*tf.matmul(x, centers)+c2upd
    ...
    reg_cost=cost+rho*(tf.nn.l2_loss(centers)+tf.nn.l2_loss(weights['v']))

    
    
    
    grad = tf.gradients(reg_cost, [weights['v']]) # this for gettting norm of gradient at optimal point
    ...
    with tf.Session() as sess:
        ...
        eng=fnormgrad[fnormgrad != np.array(None)]
